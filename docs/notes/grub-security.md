...
!!! abstract "Overview"
	...
	The steps to enable signature checking do not appear to be fully documented in the [GRUB Security Manual](https://www.gnu.org/software/grub/manual/grub/grub.html#Security) (at least as far as what's required in recent Ubuntu desktop releases), which is why this repository and the tool [grub-mksignedboot.sh](https://github.com/straysheep-dev/grub-security/blob/main/grub-mksignedboot.sh) were created. This tool will do everything outlined in this README automatically, and can be run again to update signatures.

<!-- more -->

!!! note "[grub-mksignedboot.sh](https://github.com/straysheep-dev/grub-security/blob/main/grub-mksignedboot.sh)"
...
!!! warning "Firmware vs Bootloader"
...
!!! note "Bootloader Path"

	*The path `/boot/efi/EFI/ubuntu/` is used as this is the path on both 20.04 and 22.04. This path may change depending on the OS you're using.*
...
- [Stack Exchange: Enabling Signature Checking in GRUB](https://unix.stackexchange.com/questions/531992/enabling-check-signatures-in-grub)

!!! warning "Verifiers Module"

	Check which version of GRUB you have installed, as of 2.06 the `verifiers` module may no longer need preloaded (This could vary by system and needs tested).
...
!!! warning "Updates to GRUB Mean Updates to Your Signatures"

	Anytime you change or update GRUB, it's config, the kernel, or any other boot components, you need to recompile the custom GRUB binary and write new signatures for all the boot files.
...
## Add an Admin User to GRUB

Next, **Add a user and password to `/etc/grub.d/40_custom`**.
